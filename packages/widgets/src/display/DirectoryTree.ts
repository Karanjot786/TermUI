...
export class DirectoryTree extends Widget {
    ...
    constructor(options: DirectoryTreeOptions, style: any = {}) {
        super(style);
    ...
    handleKey(key: string) {
        ...
        switch (key) {
            ...
            case 'ArrowLeft': {
                ...
                if (current.node.type === 'dir' && this._expanded.has(key)) {
                    this._toggle(current.node, current.path);
                }
                break;
            }
    ...
    protected _renderSelf(screen: any) {
        const rect = this._getContentRect();
        const width = rect.width;

        const unicode = true;
        const fileIcon = unicode ? '📄' : '[F]';
        const dirIcon = unicode ? '📁' : '[D]';
