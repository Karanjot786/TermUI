...
describe('Sparkline', () => {
    ...
    it('showRange renders min-max label on row 1', async () => {
        ...
        // min=10, max=90 — the range string "10–90" must appear on row 1
        expect(row1).toContain('10');
        expect(row1).toContain('90');
    });

    it('renders without error and blank row on empty data', async () => {
        vi.stubEnv('NO_UNICODE', '');
        vi.stubEnv('TERM', '');
        vi.resetModules();
        const { Screen } = await import('@termuijs/core');
